...
// Contract addresses
const IDENTITY_REGISTRY_ADDRESS = (process.env
...
const REPUTATION_MANAGER_ADDRESS = (process.env
  .NEXT_PUBLIC_REPUTATION_MANAGER_ADDRESS ||
  "0x0000000000000000000000000000000000000000") as Address;

// TheDesk app ID for ERC-8004
const THEDESK_APP_ID =
  "0x" + Buffer.from("otc-desk-otc").toString("hex").padEnd(64, "0");
...
export async function checkUserBan(
  ...
  try {
    ...
    try {
      ...
      console.log(
        "[ERC-8004] User not registered in Identity Registry, allowing access",
      );
      return { allowed: true };
    }
...
export async function getUserReputation(
  ...
  if (
    REPUTATION_MANAGER_ADDRESS === "0x0000000000000000000000000000000000000000"
  ) {
