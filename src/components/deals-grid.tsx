...
interface TokenGroupWithDataProps {
	...
	// API returns flat Token objects, not { token, marketData } wrapper
	token: Token | null;
}
...
const TokenGroupWithData = memo(function TokenGroupWithData({
	...
	// If token is null/undefined after loading, that's a data integrity issue
	if (!token) {
		throw new Error(
...
export function DealsGrid({ filters, searchQuery = "" }: DealsGridProps) {
	...
	const filteredGroups = useMemo(() => {
		...
		return tokenGroups.filter((group) => {
			...
			// API returns flat Token objects directly
	...
	// Check errors first (before loading) to avoid showing loading state forever on error
	if (consignmentsError || tokensError) {
		return (
			<div className="text-center py-12">
				<p className="text-red-600 dark:text-red-400">
					Failed to load deals. Please try again. {consignmentsError?.message} {tokensError?.message}
				</p>
			</div>
		);
	}
