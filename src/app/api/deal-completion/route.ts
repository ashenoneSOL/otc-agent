...
export async function GET(request: NextRequest) {
  ...
  // Enrich deals using pre-fetched data, filtering out incomplete deals
    ...
    .map((deal) => {
      ...
      if (tokenId) {
        ...
        if (!hasSymbol || !hasName) {
          ...
          if (token) {
            // Token metadata must exist if token is found
            if (!token.symbol || !token.name) {
              console.warn(
                `[Deal Completion GET] Token ${tokenId} missing symbol/name in database - skipping deal`,
              );
              return null;
            }
      ...
      // Skip deals with missing required metadata instead of throwing
      // This allows valid deals to still be returned
      const dealId = deal.quoteId ?? deal.id ?? "unknown";
      if (!tokenSymbol || !tokenName) {
        console.warn(
          `[Deal Completion GET] Deal ${dealId} missing tokenSymbol/tokenName - skipping`,
        );
        return null;
      }
