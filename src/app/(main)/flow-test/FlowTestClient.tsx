...
export default function FlowTestClient() {
	...
	// Require Solana context - throws if not available (for use in handlers)
	const requireSolanaContext = useCallback(() => {
		const ctx = getActiveSolanaContext();
		if (!ctx.publicKey) {
			throw new Error("No Solana public key available");
		}
		return ctx as { signer: typeof ctx.signer; publicKey: string };
	...
	// Track if user has confirmed their wallet choice
