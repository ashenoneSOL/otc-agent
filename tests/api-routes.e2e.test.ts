...
function assertValidationError(data: ApiErrorResponse, expectedMessage: string): void {
  const hasError =
    ...
    data.error.toLowerCase().includes(expectedMessage.toLowerCase());
  const hasDetails =
    ...
    data.details.some((d) => {
      ...
      return (
        (typeof d.message === "string" &&
          d.message.toLowerCase().includes(expectedMessage.toLowerCase())) ||
...
describe("API Routes Integration Tests", () => {
  ...
  describe("GET /api/native-prices", () => {
    test(
      "returns prices for native tokens",
      async () => {
  ...
  describe("GET /api/token-prices", () => {
    ...
    test(
      "returns prices for Solana tokens",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("GET /api/token-pool-check", () => {
    ...
    test(
      "handles Solana chain parameter",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("GET /api/pool-prices/geckoterminal", () => {
    test(
      ...
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/pool-prices/geckoterminal`,
        );
        expect(status).toBe(400);
      },
      TEST_TIMEOUT,
    ...
    test(
      "returns 400 for invalid network",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(
  ...
  describe("GET /api/pool-prices/coingecko-token", () => {
    test(
      ...
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/pool-prices/coingecko-token`,
        );
        expect(status).toBe(400);
      },
      TEST_TIMEOUT,
    ...
    test(
      "returns 400 for unsupported network",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(
    ...
    test(
      "proxies CoinGecko token info API",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<unknown>(
  ...
  describe("GET /api/tokens", () => {
    ...
    test(
      "supports chain filter",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<TokenResponse>(
          `${BASE_URL}/api/tokens?chain=base`,
        );
        expect(status).toBe(200);
        expect(data.success).toBe(true);
        expect(Array.isArray(data.tokens)).toBe(true);
  ...
  describe("GET /api/tokens/batch", () => {
    test(
      "returns 200 without ids param (empty batch)",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<TokenBatchResponse>(
    ...
    test(
      "returns empty object for empty ids",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<TokenBatchResponse>(
          `${BASE_URL}/api/tokens/batch?ids=`,
        );
        expect(status).toBe(200);
        expect(data.success).toBe(true);
        if (data.success) {
    ...
    test(
      ...
      async () => {
        ...
        const { status, data } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/tokens/batch?ids=${ids}`,
        );
        expect(status).toBe(400);
  ...
  describe("GET /api/tokens/by-symbol", () => {
    test(
      "returns 400 without symbol param",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("GET /api/tokens/addresses", () => {
    test(
      ...
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<AddressesResponse>(
          `${BASE_URL}/api/tokens/addresses`,
        );
        expect(status).toBe(200);
        expect(data.success).toBe(true);
    ...
    test(
      ...
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<AddressesResponse>(
          `${BASE_URL}/api/tokens/addresses?chain=solana`,
        );
        expect(status).toBe(200);
        expect(data.success).toBe(true);
        expect(Array.isArray(data.addresses)).toBe(true);
  ...
  describe("GET /api/tokens/decimals", () => {
    ...
    test(
      "returns 400 for invalid EVM address format",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
    ...
    test(
      "returns 400 for invalid chain parameter",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
    ...
    test(
      "returns 400 when address format does not match chain",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
    ...
    test(
      "fetches Solana token decimals with valid address",
      async () => {
        if (skipIfNoServer()) return;
    ...
    test(
      "fetches EVM token decimals with valid address",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<DecimalsResponse>(
  ...
  describe("GET /api/evm-balances", () => {
    ...
    test(
      "returns 400 for unsupported chain",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(
    ...
    test(
      "returns token balances for valid address",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("GET /api/solana/update-price", () => {
    test(
      "returns 400 without tokenMint param",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/solana/update-price`,
        );
        expect(status).toBe(400);
        expect(data.error).toBeDefined();
      },
  ...
  describe("POST /api/consignments", () => {
    ...
    test(
      "returns 400 when tokenId is missing",
      async () => {
        if (skipIfNoServer()) return;
    ...
    test(
      "returns 400 when Solana consignment missing contractConsignmentId",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(`${BASE_URL}/api/consignments`, {
          method: "POST",
    ...
    test(
      "returns 400 for invalid BPS values",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(`${BASE_URL}/api/consignments`, {
          method: "POST",
  ...
  describe("GET /api/tokens/[tokenId]", () => {
    test(
      ...
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/tokens/invalid-id`,
        );
        expect(status).toBe(400);
        expect(data.error).toBeDefined();
      },
  ...
  describe("GET /api/market-data/[tokenId]", () => {
    test(
      "accepts valid token-chain-address format",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<unknown>(
  ...
  describe("POST /api/deal-completion", () => {
    ...
    test(
      "returns 400 for invalid action enum value",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/deal-completion`,
    ...
    test(
      "returns 400 for complete action without tokenId",
      async () => {
        if (skipIfNoServer()) return;
    ...
    test(
      "accepts share action without tokenId/consignmentId",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("POST /api/otc/approve", () => {
    ...
    test(
      "returns 400 for invalid chain value",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(`${BASE_URL}/api/otc/approve`, {
          method: "POST",
    ...
    test(
      "returns 400 for invalid Solana offerAddress format",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(`${BASE_URL}/api/otc/approve`, {
          method: "POST",
    ...
    test(
      "accepts valid numeric string offerId",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("POST /api/solana/withdraw-consignment", () => {
    ...
    test(
      "returns 400 for invalid Solana address",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/solana/withdraw-consignment`,
    ...
    test(
      "returns 400 for EVM address instead of Solana",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/solana/withdraw-consignment`,
  ...
  describe("POST /api/solana/claim", () => {
    ...
    test(
      "returns 400 for invalid Solana addresses",
      async () => {
        if (skipIfNoServer()) return;
        const { status, data } = await fetchJson<ApiErrorResponse>(`${BASE_URL}/api/solana/claim`, {
          method: "POST",
    ...
    test(
      "returns 404/500 for valid addresses but non-existent offer",
      async () => {
        if (skipIfNoServer()) return;
  ...
  describe("Edge Cases and Boundary Conditions", () => {
    test(
      "handles empty string params gracefully",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(
          `${BASE_URL}/api/tokens/decimals?address=&chain=`,
        );
        expect(status).toBe(400);
      },
      TEST_TIMEOUT,
    ...
    test(
      "handles numeric overflow in amounts",
      async () => {
        if (skipIfNoServer()) return;
        const { status } = await fetchJson<ApiErrorResponse>(`${BASE_URL}/api/consignments`, {
          method: "POST",
