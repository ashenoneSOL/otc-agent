...
describe("React Query Hooks Integration Tests", () => {
  ...
  describe("useToken behavior (via /api/tokens/[tokenId])", () => {
    ...
    test("caches token data (multiple requests same result)", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("useMarketData behavior (via /api/market-data/[tokenId])", () => {
    test("returns 404 or null for non-existent token", async () => {
      if (skipIfNoServer()) return;
    ...
    test("accepts valid tokenId format", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("usePoolCheck behavior (via /api/token-pool-check)", () => {
    ...
    test("skips pool check for Solana (returns success)", async () => {
      if (skipIfNoServer()) return;
    ...
    test("validates EVM address format", async () => {
      if (skipIfNoServer()) return;
    ...
    test("checks pool for valid EVM token", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("useNativePrices behavior (via /api/native-prices)", () => {
    test("returns prices object", async () => {
      if (skipIfNoServer()) return;
    ...
    test("returns positive numbers for available prices", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("useConsignment behavior (via /api/consignments/[id])", () => {
    ...
    test("supports callerAddress query param", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("useTokenLookup behavior (via /api/token-lookup)", () => {
    ...
    test("respects chain parameter", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("Mutation endpoint validation", () => {
    test("POST /api/consignments validates required fields", async () => {
      if (skipIfNoServer()) return;

      const { status, data } = await fetchJson<{ error?: string }>(`${BASE_URL}/api/consignments`, {
        method: "POST",
    ...
    test("POST /api/deal-completion validates action field", async () => {
      if (skipIfNoServer()) return;
    ...
    test("POST /api/otc/approve validates chain field", async () => {
      if (skipIfNoServer()) return;

      const { status, data } = await fetchJson<{ error?: string }>(`${BASE_URL}/api/otc/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    test("POST /api/solana/claim validates Solana addresses", async () => {
      if (skipIfNoServer()) return;

      const { status, data } = await fetchJson<{ error?: string }>(`${BASE_URL}/api/solana/claim`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    test("POST /api/solana/withdraw-consignment validates addresses", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("Edge cases and boundary conditions", () => {
    test("handles empty tokenId gracefully", async () => {
      if (skipIfNoServer()) return;
    ...
    test("handles missing Content-Type header for POST", async () => {
      if (skipIfNoServer()) return;
  ...
  describe("Cache invalidation patterns", () => {
    test("token list endpoint returns consistent structure", async () => {
      if (skipIfNoServer()) return;
    ...
    test("consignment list endpoint returns consistent structure", async () => {
      if (skipIfNoServer()) return;
    ...
    test("token batch endpoint returns map structure", async () => {
      if (skipIfNoServer()) return;
