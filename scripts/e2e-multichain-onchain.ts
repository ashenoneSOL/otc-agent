...
const mainnetSolanaConfigRaw = JSON.parse(
...
async function testEvmChain(chainKey: string): Promise<void> {
  ...
  results.push({
    ...
    success: true,
    details:
      activeConsignment && consignmentDetails
        ? {
  ...
  if (elizaosMint) {
    ...
    if (registryInfo) {
      ...
      const tokenMint = new PublicKey(
        registryInfo.data.subarray(regOffset + 32, regOffset + 64),
      ).toBase58();
      // decimals: offset 72
    ...
    const elizaosTokenAccount = getAssociatedTokenAddressSync(elizaosMintPubkey, wallet.publicKey);
